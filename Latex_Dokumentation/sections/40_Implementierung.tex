...
In diesem Projekt wurde die Schachengine "Carballo" verwendet. \unsure{Kommt das
hier überhaupt hin? Wenn ja, mehr?} \info{Joa, vielleicht etwas mehr
Überleitung}
...
Außerdem implementiert \code{Savegame} das \emph{Interface} 
\code{Comparable<Savegame>} und überschreibt die \code{equals()}-Methode damit
\code{SavegameAdapter} diese zum Sortierenbenutzen kann.
...
Weitere Methoden die \code{Savegame} besitzt, sind zum einen die \code{update()}
und die getDateString()-Methode. Wobei erstere, wie der Name schon sagt, für das
...
genutzt werden um Spielstände in einem \code{Recyclerview} anzuzeigen und zu laden.
